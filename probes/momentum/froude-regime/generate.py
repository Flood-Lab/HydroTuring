...
STATIC = {
    ...
    # The fixed vertical datum `stage` is reported against. It is a real
    # elevation, not the bed and not zero: the contract is that a gauge reads a
    # water-surface elevation on a datum that does not move, so the criterion
    # forms the flow depth as `stage - bed_elevation_m` rather than reading the
    # level as a depth. A datum of zero would be the same conflation in
    # disguise — it would let an adapter that never read the key score exactly
    # as one that did.
    "bed_elevation_m": 120.0,
}
