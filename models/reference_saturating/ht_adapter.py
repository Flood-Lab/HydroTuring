...
MODEL = {"name": "reference_saturating", "version": "1.0.0"}


EVAP_SHAPE = 0.5  # soil moisture at which evaporation reaches its potential rate
MAX_DAILY_RUNOFF = 25.0  # mm/day, the most it will ever report
...
def simulate(forcing, static, dt_days=1.0):
    ...
    for step in forcing:
        ...
        soil -= surface
        # The only line that differs from reference_bucket. Runoff above what
        # the model has ever seen in a day is not reported: it leaves.
        surface = min(surface, MAX_DAILY_RUNOFF * dt_days)
