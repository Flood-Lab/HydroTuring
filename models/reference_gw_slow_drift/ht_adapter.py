#!/usr/bin/env python3
"""Exact bucket physics with a deliberately drifting groundwater reporter."""
...
BIAS_MM_PER_DAY = 0.001
COLUMNS = ["time", "pr", "evspsbl", "mrro", "mrso", "snw", "canopy", "gw", "channel"]
MODEL = {"name": "reference_gw_slow_drift", "version": "1.0.0"}
...
def simulate(forcing, static):
    ...
    for index, step in enumerate(forcing):
        ...
        rows.append({
            ...
            "mrso": soil, "snw": swe, "canopy": canopy,
            "gw": BIAS_MM_PER_DAY * (index + 1), "channel": 0.0,
        })
    return rows
...
def main():
    ...
    if request.get("timestep", "PT1D") != "PT1D":
        raise SystemExit("reference_gw_slow_drift supports PT1D only")
