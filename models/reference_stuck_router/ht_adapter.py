#!/usr/bin/env python3
"""HydroTuring adapter for reference_stuck_router: the exact bucket with one deliberate fault.

The kernel sums to 0.9. `channel` is reported as generated minus released,
which is what the contract asks, and it grows forever.
"""
...
MODEL = {"name": "reference_stuck_router", "version": "1.0.0"}
...
def simulate(forcing, static, dt_days=1.0):
    ...
    generated = []
    cum_gen = cum_out = 0.0
    # A three-day kernel that sums to 0.9: a tenth of the flow is never released.
    W = [0.45, 0.3, 0.15]
