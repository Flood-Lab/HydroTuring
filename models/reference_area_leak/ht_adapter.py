#!/usr/bin/env python3
"""HydroTuring adapter for reference_area_leak: the exact bucket with one deliberate fault.

A share of runoff proportional to the catchment area vanishes. The same
weather on a ten times larger catchment gives a different depth.
"""
...
MODEL = {"name": "reference_area_leak", "version": "1.0.0"}
...
def simulate(forcing, static, dt_days=1.0):
    ...
    canopy = 0.0
    rows = []
    leak = min(0.5, float(static.get("area_km2", 0.0)) / 5000.0)
    ...
    for step in forcing:
        ...
        rows.append({
            ...
            "mrro": (1.0 - leak) * (surface + baseflow) / dt_days,
            "mrso": soil,
            "snw": swe,
            "canopy": canopy,
        })
