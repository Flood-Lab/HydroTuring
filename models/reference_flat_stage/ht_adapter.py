#!/usr/bin/env python3
"""HydroTuring adapter for reference_flat_stage: the routed bucket whose gauge
never moves.

The routing, the discharge and the stores are all honest. The fault is that
the stage is reported as a constant — a plausible-looking level handed back
for every step regardless of what the reach is doing. It is what a model that
stubs out its diagnostic, or fills the column from a default, produces. There
is no rating curve in it because the gauge carries no information, which is
what the degeneracy check on the stage is there to find.
"""
...
# The nominal level the gauge reports no matter what.
FLAT_STAGE_M = 1.20
...
def simulate(forcing, static, dt_days=1.0):
    ...
    generated = []
    cum_gen = cum_out = 0.0
    W = [0.5, 0.3, 0.2] if dt_days >= 1.0 else [0.6, 0.25, 0.15]
