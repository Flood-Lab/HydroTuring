#!/usr/bin/env python3
"""HydroTuring adapter for reference_noise_sink.

The exact bucket, misreporting its evaporation by a seeded plus or minus
thirty percent and declaring the difference as a groundwater exchange. The
reported budget closes to floating point; the declared flux reverses direction
with the weather, because what it absorbs is the day's accounting error.
"""
...
MODEL = {"name": "reference_noise_sink", "version": "1.0.0"}
...
def simulate(forcing, static, dt_days=1.0, seed=0):
    ...
    for step in forcing:
        ...
        rows[-1]["gwex"] = (rows[-1]["evspsbl"] - true_et) - removed / dt_days
    return rows
