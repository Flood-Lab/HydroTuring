#!/usr/bin/env python3
"""HydroTuring adapter for reference_evolving_exchange.

The exact bucket with a head-driven boundary exchange whose internal head
moves: `S dh/dt = C (H_ext - h)`, so water entering across the boundary raises
the catchment's own head and the exchange dies away as the two heads meet.
With S = 10 mm per metre and C = 2 mm per metre per day the time constant is
five days, far shorter than the spinup, so the model is in equilibrium with
the prescribed head when scoring begins and answers a 0.5 m shift with the
water needed to raise its own head by 0.5 m — about S * 0.5 = 5 mm — over the
first weeks of the scored record, and with nearly nothing thereafter.

It is the positive control that a constant-head control cannot be. A control
whose internal head never moves answers a shift with a sustained flux for the
whole record and would pass under any timing; this one exposes the failure of
applying the shift during spinup (the answer would fall outside the scored
window) and sets the calibration of the response floor, because its response
is the small one a real aquifer gives. It must pass.
"""
...
MODEL = {"name": "reference_evolving_exchange", "version": "1.0.0"}

# Boundary conductance and the storativity of the catchment's own head:
# S dh/dt = C (H_ext - h). Time constant S / C = 5 days.
CONDUCTANCE_MM_PER_M_DAY = 2.0
STORATIVITY_MM_PER_M = 10.0
INITIAL_HEAD_M = 10.0
