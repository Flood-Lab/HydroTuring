#!/usr/bin/env python3
"""HydroTuring adapter for reference_overflowing: the exact bucket with one deliberate fault.

Runoff is reported as what the stores released plus 80 percent of the
day's rain again. Stores and evaporation are exact, so the extra water has
no source: over a record the runoff exceeds the rain, and the runoff bound
catches it with nothing but the hydrograph.
"""
...
MODEL = {"name": "reference_overflowing", "version": "1.0.0"}
...
def simulate(forcing, static, dt_days=1.0):
    ...
    for step in forcing:
        ...
        rows.append({
            ...
            "mrro": (surface + baseflow + 0.8 * pr) / dt_days,  # 80 percent of the rain again, from nowhere
            "mrso": soil,
            "snw": swe,
            "canopy": canopy,
        })
