#!/usr/bin/env python3
"""HydroTuring adapter for reference_sublimating: the exact bucket with one deliberate fault.

Forty percent of snowfall vanishes. Rain is conserved exactly, so the
fault only shows when the same water arrives as snow.
"""
...
def simulate(forcing, static, dt_days=1.0):
    ...
    for step in forcing:
        ...
        swe += 0.6 * snowfall  # the rest sublimates, unreported
