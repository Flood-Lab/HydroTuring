...
def simulate(forcing: list[dict], static: dict) -> list[dict]:
    ...
    gw_to_sw + sw_to_gw != gwex on those steps. The head lags a smoothed
    river stage rather than equilibrating to today's stage, so it genuinely
    runs both above and below the river bottom over the record; the
    untouched steps still carry a real negative gw_to_sw, so
    exchange_directions sees a genuine reverse-direction magnitude and only
    exchange_components, which checks the running sum against gwex, catches
    the error."""
    ...
    for step, row in enumerate(forcing):
        ...
        # MODFLOW's RIV convention and the repo's gwex-positive-into-aquifer
        # rule. Below the river bottom, MODFLOW's RIV package caps the
        # exchange at the bottom elevation instead of the (unreachable) head.
        ...
        gwex = gw_to_sw_true + sw_to_gw

        gw_mm += recharge_mm + gwex

        # Deliberate sign error, on every other step only: report the
        # aquifer-to-river magnitude without negating it, so the reported
        # components no longer sum to the correctly-signed gwex on those
        # steps. The untouched steps keep a real gw_to_sw <= 0, so
