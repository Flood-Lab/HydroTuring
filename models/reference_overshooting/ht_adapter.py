#!/usr/bin/env python3
"""HydroTuring adapter for reference_overshooting: the exact bucket with one deliberate fault.

A derivative term makes the hydrograph overshoot: more water on one day
means less reported flow on the next.
"""
...
MODEL = {"name": "reference_overshooting", "version": "1.0.0"}
...
def simulate(forcing, static, dt_days=1.0):
    ...
    for step in forcing:
        ...
        runoff = surface + baseflow
        sharpened = max(0.0, runoff + 0.8 * (runoff - previous))
        previous = runoff
        rows.append({
            "time": step["time"],
            "pr": pr_rate,
            "evspsbl": (canopy_evap + soil_evap) / dt_days,
            "mrro": sharpened / dt_days,
            "mrso": soil,
            "snw": swe,
            "canopy": canopy,
        })
