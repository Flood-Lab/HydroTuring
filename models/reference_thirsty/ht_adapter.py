#!/usr/bin/env python3
"""HydroTuring adapter for reference_thirsty: the exact bucket with one deliberate fault.

Soil evaporation is a fixed fraction of the store per day whatever the
potential rate. Mass closes; the energy ceiling and floor do not.
"""
...
MODEL = {"name": "reference_thirsty", "version": "1.0.0"}
...
def simulate(forcing, static, dt_days=1.0):
    ...
    for step in forcing:
        ...
        soil -= baseflow
        soil_evap = min(soil, 0.02 * soil * dt_days)  # demand is never consulted
        soil -= soil_evap
