...
| `stage` | water-surface elevation measured from the same vertical datum as `bed_elevation_m`; water depth is `stage - bed_elevation_m`. If a case does not supply `bed_elevation_m`, the local channel bed is datum zero. Stage is a diagnostic, declared under `emits.diagnostics`, never a storage any budget is differenced over | m |
