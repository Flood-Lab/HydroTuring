...
| `stage` | the water level a gauge in the reach would read, **measured from the reach bed**: the datum is the bed, so an absolute level, or a level above any other zero, is a different quantity and not this variable; derived from the reach's own water; a diagnostic, declared under `emits.diagnostics`, never a storage any budget is differenced over | m |
