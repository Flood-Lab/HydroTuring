...
| [`wflow_sbm`](models/wflow_sbm) | submitted | Deltares' Wflow.jl SBM: a soil column with unsaturated and saturated stores, interception, snow and kinematic-wave routing, adapted in Julia and run on one representative cell at the resolution of Wflow's Moselle model. | **FAIL (INCOMPLETE)**, 13 of 18 probes passed. It reports no heat fluxes, so the three energy-flux probes cannot ask it anything; its water budget closes to 2e-4 of the rain and it passes the area, routing, step, calendar, causality, extreme-rain, phase and warming probes. Both failures come from its soil column below the Moselle rooting depth: after a wet month the soil has transpired its extra water down to the depth the roots reach, so a storm on unsaturated soil runs off no more than after a dry one, and once the root zone dries in a rainless spell, drainage from the unsaturated store raises the water table and lateral flow rises with no rain. Both move with how the stated soil capacity is mapped onto soil thickness and roots |
