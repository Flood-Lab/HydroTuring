...
| [`lisflood`](models/lisflood) | submitted | LISFLOOD 5.0.0, the EC Joint Research Centre's distributed model behind EFAS and GloFAS, stepped through its own Python framework on a one-cell domain the size of the catchment, reporting every store its own water balance module counts. | **FAIL (ERROR)**, 13 of 19 probes passed. Its budget closes to 1e-13 mm per step, and it reports no heat fluxes, so the three energy-flux probes cannot ask it anything. It fails the step probe because its potential infiltration is a pore-space storage multiplied by the step length, so rain falling within hours runs off at the hourly step (13.1% of the rain between PT1H and PT1D), and the area probe because its kinematic-wave routing is nonlinear in volume, so the same depth told as ten times the area travels on a different schedule. The precipitation-counterfactual probe's ten-year variants take 52–61 s each under amd64 emulation against a 60 s budget, and the harness stopped it on a timeout; run outside the limit, every criterion passes |
