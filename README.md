...
Twenty-eight: seventeen under mass, seven under energy and four under momentum.
...
| `reference_unreported_inflow` | broken | routes exactly and conserves water, then adds a bounded inflow the model never reports, so the reach rises on rainless steps without ever nearing the store bound | caught by `recession_drainage`, passed by `routing_conservation` |
