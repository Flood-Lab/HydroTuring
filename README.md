...
| [`cwatm`](models/cwatm) | submitted | CWatM 1.11, IIASA's Community Water Model and an ISIMIP global hydrological model, run on one grid cell with every store it carries reported. | **FAIL (INCOMPLETE)**, 12 of 18 probes passed. No energy fluxes, so INCOMPLETE on the three energy-flux probes; its budget closes to 0.03%, and it fails on a groundwater reservoir with no dt that drains 24 times too fast at an hourly step (52% of the rain), a preferential-flow term that sends a storm on wet soil to groundwater faster than it adds runoff (a 0.29 mm/day dip), and evaporation on wet soil at 0.70 of demand, the ceiling its crop coefficients set; the last two verdicts move with the package's land-cover and crop-coefficient choices |
