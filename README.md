...
| [`wflow_sbm`](models/wflow_sbm) | submitted | Deltares' Wflow.jl SBM: a soil column with unsaturated and saturated stores, interception, snow and kinematic-wave routing, adapted in Julia and run on a one-cell catchment. | **FAIL (INCOMPLETE)**, 11 of 18 probes passed. It reports no heat fluxes, so the three energy-flux probes cannot ask it anything; its water budget closes to 3e-6 of the rain and it passes the step, calendar, causality, extreme-rain, phase and warming probes. Two failures come from the one cell a lumped catchment is given, whose hillslope is as long as the catchment: a runoff depth that changes with the area it is told, and an overland kinematic wave that holds water for months at low flow. Two are the SBM column under every setting tried: a storm on unsaturated soil runs off almost nothing however wet the soil was, because the wetter column evaporates its extra water, and on one seed a two-year drain from the saturated store runs too steadily to count as varying or, where lateral flow carries the runoff, rises when evaporation stops in winter |
