...
every probe that can ask them anything; five of the six energy probes and
...
| [`mass/gw-sw-exchange-consistency`](probes/mass/gw-sw-exchange-consistency) | mass | A model reporting groundwater-river exchange: do recharge, the net exchange and the two signed directional components, and aquifer storage all agree? | `reference_exchange_sign_error` |
...
| [`modflow6`](models/modflow6) | physical | MODFLOW 6.7.0, the USGS's modular groundwater model: a transient one-river-cell aquifer run through flopy, reporting the RIV and STO budget terms as signed exchange and groundwater storage. | must pass `mass/gw-sw-exchange-consistency`, the only probe that can ask it anything; **PASS**, 1 of 1 |
| `reference_bucket` | exact | conserves water exactly by construction | must pass every probe that can ask it anything; N/A on the five energy probes and the groundwater-exchange probe that need outputs it does not report |
