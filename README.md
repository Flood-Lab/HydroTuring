...
| [`energy/snowmelt-energy-water`](probes/energy/snowmelt-energy-water) | energy | A pack built, ripened, then melted out over sixty dry days: does the surface energy budget pay the latent heat of fusion the reported melt demanded? | `reference_coupled`, `reference_two_head` |
...
`energy/surface-energy-closure` and `energy/snowmelt-energy-water`,
...
| `reference_coupled` | exact | the bucket with snow sublimation and a surface energy budget: every kilogram converted at the latent heat of the phase it actually underwent | must pass every criterion of the three energy-flux probes it is not the counterexample on; it melts by degree-day and its energy budget never charges the fusion, which is what `energy/snowmelt-energy-water` catches it by; supports daily and hourly steps |
| `reference_snow_energy` | exact | `reference_coupled` with melt bought from the pack's own surface energy budget, `max(0, Rn - H - LE - G) / lambda_f`, rather than from a degree-day factor | must pass every criterion of `energy/snowmelt-energy-water`; supports daily and hourly steps |
