"""Tests for the driver-ablation probe: same rain, warmer air.

The expectation is a sign, not a closure: more evaporative demand over the
same water must leave less runoff. These check that the generator changes
only the driver, that the criterion reads the sign and the size of the
response against the added demand, that the exact model responds the way
physics says while a temperature-blind model does not, and that a submitted
model's window is widened to the year the expectation needs.
"""
...
def test_only_the_driver_changes(probe):
    control = build_case(probe, 7, "control")
    warmer = build_case(probe, 7, "warmer")
    assert control.forcing["pr"].equals(warmer.forcing["pr"])
    assert control.forcing["time"].equals(warmer.forcing["time"])

    spinup = control.spinup_steps
    assert control.forcing["tas"][:spinup].equals(warmer.forcing["tas"][:spinup])
    delta = (warmer.forcing["tas"] - control.forcing["tas"])[spinup:]
    np.testing.assert_allclose(delta, 3.0, atol=1e-5)
    assert (warmer.forcing["pet"][spinup:] >= control.forcing["pet"][spinup:]).all()
    assert (warmer.forcing["pet"][spinup:] > control.forcing["pet"][spinup:]).mean() > 0.95
    # A rain-dominated catchment: snow timing must not confound the sign.
    assert (control.forcing["tas"] < 0).mean() < 0.01


def test_exact_model_loses_runoff_and_gains_evaporation(probe):
    ...
    shares = response.diagnostics["shares"]
    assert shares["mrro"] < -0.1
    assert shares["evspsbl"] > 0.1
    # Over ten years storage change is small, so what evaporates is what
    # stopped running off.
    assert abs(shares["mrro"] + shares["evspsbl"]) < 0.1
    assert response.diagnostics["added_demand_mm"] > 1000
