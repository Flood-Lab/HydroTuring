...
def groundwater_balance(run: RunResult, probe: ProbeSpec, params: dict) -> CriterionResult:
    """Check recharge - net groundwater-to-river exchange = storage change."""
    ...
    allowed = np.maximum(rel_tol * np.maximum(np.abs(recharge), 1.0e-12), abs_tol)
...
def exchange_directions(run: RunResult, probe: ProbeSpec, params: dict) -> CriterionResult:
    ...
    forward = float(np.maximum(w.volume(w.table["gw_to_sw"]), 0.0).sum())
    reverse = float(-np.minimum(w.volume(w.table["sw_to_gw"]), 0.0).sum())
