...
def soil_heat_storage(run: RunResult, probe: ProbeSpec, params: dict) -> CriterionResult:
    ...
    # Finite output columns can still overflow in differences, products or
    # reductions. Reject the failed calculation before inf <= inf can pass.
    with np.errstate(over="ignore", invalid="ignore"):
        temperature_change = temperatures - previous
        net_input = top_flux - bottom_flux
        storage_rate = capacity * temperature_change / (w.dt_days * 86400.0)
        residual = net_input - storage_rate
    finite_derived = (
        np.isfinite(temperature_change) & np.isfinite(net_input)
        & np.isfinite(storage_rate) & np.isfinite(residual)
    )
    if not finite_derived.all():
        n_bad = int((~finite_derived).sum())
        return CriterionResult(
            name="soil_heat_storage", status=FAIL,
            message=f"non-finite derived soil heat values on {n_bad} scored intervals",
            diagnostics={"non_finite_steps": n_bad, "non_finite_stage": "derived"},
        )

    zero_boundary_fluxes = bool((top_flux == 0).all() and (bottom_flux == 0).all())
    zero_temperature_change = bool((temperature_change == 0).all())
    blocks = []
    for label, start, stop in phases:
        with np.errstate(over="ignore", invalid="ignore"):
            ...
            mean_storage = float(np.abs(storage_rate[start:stop]).mean())
        relative_allowance = threshold * mean_net
        allowance = max(relative_allowance, floor)
        slack = mean_abs / allowance
        metrics = {
            "mean_abs_w_m2": mean_abs,
            "mean_abs_net_input_w_m2": mean_net,
            "mean_abs_storage_rate_w_m2": mean_storage,
            "relative_allowance_w_m2": relative_allowance,
            "allowance_w_m2": allowance,
            "slack": slack,
        }
        non_finite_metrics = [key for key, value in metrics.items() if not np.isfinite(value)]
        if non_finite_metrics:
            return CriterionResult(
                name="soil_heat_storage", status=FAIL,
                message=f"non-finite soil heat phase metrics in {label} [{start}:{stop}]",
                diagnostics={
                    "non_finite_stage": "phase", "non_finite_phase": label,
        ...
        blocks.append({
            ...
            "mean_abs_storage_rate_w_m2": mean_storage,
            "allowance_w_m2": allowance,
            "floor_controls": floor >= relative_allowance,
            "slack": slack,
    ...
    return CriterionResult(
        ...
        message=(
            ...
            + ("; no thermal response exercised (zero boundary fluxes and temperature change)"
               if zero_boundary_fluxes and zero_temperature_change else "")
        ),
        diagnostics={
            ...
            "zero_boundary_fluxes": zero_boundary_fluxes,
            "zero_temperature_change": zero_temperature_change,
        },
    )
